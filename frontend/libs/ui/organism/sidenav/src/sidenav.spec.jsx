...
import { Item, Section } from '@compendium/frontend.ui.molecule.tree';
import React from 'react';
import { SideNav } from '.';
import { triggerPress } from '@react-spectrum/test-utils';

const flatItems = [{ name: 'Foo' }, { name: 'Bar' }, { name: 'Bob' }, { name: 'Alice' }];

const withSection = [
  { name: 'Section 1', children: [{ name: 'Foo' }, { name: 'Bar' }] },
  { name: 'Section 2', children: [{ name: 'Bob' }, { name: 'Alice' }] }
];
...
describe.skip('SideNav', function () {
  ...
  `(
    '$Name has default behavior',
    async function ({ Name, Component, ComponentSection, ComponentItem }) {
      ...
      const sideNavList = getByRole('list');
      await waitFor(() => {
      ...
      const sideNavListItems = getAllByRole('listitem');
      expect(sideNavListItems.length).toBe(4);

      const sideNavListItemLinks = getAllByRole('link');
  ...
  `(
    '$Name can pass className',
    async function ({ Name, Component, ComponentSection, ComponentItem, props }) {
  ...
  it.each`
    ...
    ${'SideNavStaticWithSections'} | ${SideNav} | ${Section}       | ${Item}
  `(
    '$Name can single select an item',
    async function ({ Name, Component, ComponentSection, ComponentItem }) {
      const spy = jest.fn();
      const { getByRole, getByText } = renderComponent(
        ...
        ComponentSection,
        ComponentItem,
        { selectionMode: 'single', onSelectionChange: spy }
      );
  ...
  `(
    ...
    async function ({ Name, Component, ComponentSection, ComponentItem }) {
      ...
      const [section1, section2] = [getByText('Section 1'), getByText('Section 2')];
  ...
  it.each`
    ...
    ${'SideNavStaticWithSections'} | ${SideNav} | ${Section}       | ${Item}
  `(
    '$Name can keep focus on first/last item',
    async function ({ Name, Component, ComponentSection, ComponentItem }) {
      const { getByRole, getAllByRole, getByText } = renderComponent(
        Name,
        Component,
  ...
  it.each`
    ...
    ${'SideNavStaticWithSections'} | ${SideNav} | ${Section}       | ${Item}
  `(
    '$Name can focus first to last/last to first item',
    async function ({ Name, Component, ComponentSection, ComponentItem }) {
      const { getByRole, getAllByRole, getByText } = renderComponent(
        Name,
        Component,
  ...
  it.each`
    ...
    ${'SideNavStaticWithSections'} | ${SideNav} | ${Section}       | ${Item}
  `(
    '$Name supports defaultSelectedKeys (uncontrolled)',
    async function ({ Name, Component, ComponentSection, ComponentItem }) {
      const { getByRole, getByText } = renderComponent(
        Name,
        Component,
        ComponentSection,
        ComponentItem,
        { selectionMode: 'single', defaultSelectedKeys: ['Bar'] }
      );
  ...
  it.each`
    ...
    ${'SideNavStaticWithSections'} | ${SideNav} | ${Section}       | ${Item}
  `(
    '$Name supports defaultSelectedKeys (controlled)',
    async function ({ Name, Component, ComponentSection, ComponentItem }) {
      const { getByRole, getByText } = renderComponent(
        Name,
        Component,
  ...
  `(
    ...
    async function ({ Name, Component, ComponentSection, ComponentItem }) {
      ...
      const { getByRole, getByText } = renderComponent(
        ...
        ComponentSection,
        ComponentItem,
        { onSelectionChange: spy, disabledKeys: ['Foo', 'Bob'] }
      );
  ...
  it.each`
    ...
    ${'SideNavStaticWithSections'} | ${SideNav} | ${Section}       | ${Item}
  `(
    '$Name does not allow empty selection',
    async function ({ Name, Component, ComponentSection, ComponentItem }) {
      const spy = jest.fn();
      const { getByRole, getByText } = renderComponent(
        Name,
        ...
        ComponentSection,
        ComponentItem,
        { selectionMode: 'single', disallowEmptySelection: true, onSelectionChange: spy }
      );
      ...
      const [bar, alice] = [getByText('Bar'), getByText('Alice')];
  ...
  it.each`
    ...
    ${'SideNavStaticWithSections'} | ${SideNav} | ${Section}       | ${Item}
  `(
    '$Name with default key does not allow empty selection',
    async function ({ Name, Component, ComponentSection, ComponentItem }) {
      const spy = jest.fn();
      const { getByRole, getByText } = renderComponent(
        Name,
      ...
      const [bar, alice] = [getByText('Bar'), getByText('Bob')];
